...
//page that inputs login data to the database
    ...
    try{// try this block of code to catch any errors
        ...
        //sql statement so that the information can be gotten from the database
        $sql = "SELECT email, password,user_id FROM users WHERE email = ?";

        //prepare and bind the sql for security against sql injection attacks
        $stmt = $conn->prepare($sql);
        $stmt -> bindParam(1, $_POST["email"]);
        $stmt->execute();
        //results gotten in an array
        $result = $stmt->fetch();

        //if there is a result do tje next piece of code
        if($result != null){
            ...
            if(password_verify($_POST["password"], $result["password"])) { // if password matches one in database do next section of code
                ...
                auditor($conn, $_POST["email"],'fla',"Login failed");
            ...
            echo "<meta http-equiv='refresh' content='2;url=register.php'>"; //redirect
            echo "<link rel='stylesheet' type='css' href='../styles.css'>";
